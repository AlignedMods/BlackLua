...
namespace BlackLua::Internal {
    ...
    void ASTDumper::DumpNodeExpr(NodeExpr* expr, size_t indentation, size_t line) {
        ...
        if (ExprMethodCall* call = GetNode<ExprMethodCall>(expr)) {
            ...
            DumpNodeExpr(call->Parent, indentation + 4, expr->Loc.Line);
            return;
        }
        ...
        if (ExprParen* paren = GetNode<ExprParen>(expr)) {
            m_Output += "ParenExpr\n";
            DumpNodeExpr(paren->Expression, indentation + 4, expr->Loc.Line);
            return;
        }
    ...
    void ASTDumper::DumpNodeStmt(NodeStmt* stmt, size_t indentation, size_t line) {
        ...
        if (StmtWhile* wh = GetNode<StmtWhile>(stmt)) {
            m_Output += "WhileStmt\n";
            DumpNodeExpr(wh->Condition, indentation + 4, stmt->Loc.Line);
            DumpNodeStmt(wh->Body, indentation + 4, stmt->Loc.Line);
            return;
        }
        ...
        if (StmtFor* fo = GetNode<StmtFor>(stmt)) {
            m_Output += "ForStmt\n";
            DumpNodeStmt(fo->Prologue, indentation + 4, stmt->Loc.Line);
            DumpNodeExpr(fo->Condition, indentation + 4, stmt->Loc.Line);
            DumpNodeExpr(fo->Epilogue, indentation + 4, stmt->Loc.Line);
            DumpNodeStmt(fo->Body, indentation + 4, stmt->Loc.Line);
            return;
        }

        if (StmtIf* i = GetNode<StmtIf>(stmt)) {
            m_Output += "IfStmt\n";
            DumpNodeExpr(i->Condition, indentation + 4, stmt->Loc.Line);
            DumpNodeStmt(i->Body, indentation + 4, stmt->Loc.Line);

            if (i->ElseBody) {
                DumpNodeStmt(i->ElseBody, indentation + 4, stmt->Loc.Line);
            }
        ...
        if (StmtReturn* ret = GetNode<StmtReturn>(stmt)) {
            m_Output += "ReturnStmt\n";
            DumpNodeExpr(ret->Value, indentation + 4, stmt->Loc.Line);
            return;
        }
    }
